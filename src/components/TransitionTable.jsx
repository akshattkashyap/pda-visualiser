      ...
      <table className="w-full text-left text-sm text-slate-300 font-mono">
        <thead className="text-xs uppercase bg-slate-900 text-slate-500 font-sans tracking-widest border-b border-white/10">
          <tr>
            ...
            <th className="px-4 py-3 font-bold text-amber-500/80">Input</th>
            <th className="px-4 py-3 font-bold text-rose-500/80">Pop</th>
            <th className="px-4 py-3 text-white/30 text-center">→</th>
            <th className="px-4 py-3">To</th>
            <th className="px-4 py-3 font-bold text-emerald-500/80">Push</th>
        ...
        <tbody className="divide-y divide-white/5 bg-black/20">
          {transitions.map((t, i) => (
            <tr key={t.id || i} className="hover:bg-white/5 transition-colors group">
              <td className="px-4 py-2 text-blue-300 font-bold">{t.from}</td>
              <td className="px-4 py-2">{t.input === 'ε' ? <span className="text-slate-500">ε</span> : t.input}</td>
              <td className="px-4 py-2">{t.stackPop === 'ε' ? <span className="text-slate-500">ε</span> : t.stackPop}</td>
              <td className="px-4 py-2 text-center text-slate-600">→</td>
              <td className="px-4 py-2 text-blue-300 font-bold">{t.to}</td>
              <td className="px-4 py-2">{t.stackPush === 'ε' ? <span className="text-slate-500">ε</span> : t.stackPush}</td>
